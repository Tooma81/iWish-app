...
export default function FriendsScreen() {
  const params = useLocalSearchParams(); // Saame kätte "share_wish" ja "wishData"
...
const handleFriendPress = async (friendId: string) => {
  if (params.action === "share_wish" && params.wishData) {
    try {
      // TypeScripti parandus: võtame esimese elemendi kui on massiiv
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          user_id: friendId,           // See saadab sõbrale
          created_by: session?.user.id, // Nüüd on see veerg andmebaasis olemas!
          came_true: false
        });
...
const renderFriendItem = ({ item }: { item: Friend }) => (
  <View style={styles.friendItem}>
    {/* Muudame selle osa TouchableOpacity-ks */}
    <TouchableOpacity 
style={[
    styles.friendInfo, 
...
<>
  ...
  {/* Kui on jagamise režiim, võid peita "lisa sõber" nupu, et fookus püsiks valikul */}
